...
class ClinicalTrialsAPIWrapper:
    ...
    def search(self, search_query: str, status: str = None, max_studies: int = 15) -> list:
        """
        检索临床试验，返回结构化的试验信息列表。
        """
        fields_to_get = [
        ...
        params = {
            'query.term': f"{search_query} {status}" if status else search_query,
    ...
    def search_and_parse(self, search_query: str, status: str = None, max_studies: int = 15) -> list:
        """
